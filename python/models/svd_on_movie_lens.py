...
RECORD_FILE_PATH = os.path.join(here, os.pardir, "records", "../records/svd_movielens_recommendation.txt")
...
def do_svd(trainingSet, start_time):
    ...
    LOGGER.info("0;Data prediction completed in '%s' minutes",
                str((time.time() - start_time) / 60))
    print("Rmse values for doing svd based recomm on movielens data is " + str(accuracy.rmse(predictions)))
    return predictions
...
def do_svd_on_movie_lens(uid, topk=5, refresh="no"):
    ...
    if refresh == "no" and (os.path.isfile(RECORD_FILE_PATH)):
        ...
        predictions = do_svd(trainingSet, start_time)
